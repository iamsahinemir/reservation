...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['update_role']) && !isset($_POST['update_status'])) {
    ...
    // Gerekli verilerin boş olup olmadığını kontrol et
    if (!empty($name) && !empty($email) && !empty($s_number) && !empty($password) && !empty($role)) {
                ...
                <body>
                    <div class='container'>
                        ...
                        <div class='content'>
                            ...
                            <p>CCIP Laboratuvar Rezervasyon sistemimize kayıt olduğunuz için teşekkür ederiz. Hesabınıza giriş yapmak için <a href='http://localhost/randevu/ccip/'>buraya</a> tıklayın.</p>
                            <p>Saygılarımızla,</p>
                            <p><strong>CCIP Laboratuvar Ekibi</strong></p>
                        </div>
                        <div class='footer'>
        ...
        $_SESSION['status'] = "Lütfen tüm alanları doldurunuz.";
    }
}

// Kullanıcıların hesap durumunu güncelleme işlemi
if (isset($_POST['update_status'])) {
    ...
    if ($stmt->execute()) {
        $_SESSION['status'] = "Hesap durumu başarıyla güncellendi.";
    } else {
        $_SESSION['status'] = "Hesap durumu güncellenemedi.";
    }
...
// Kullanıcıların rollerini güncelleme işlemi
if (isset($_POST['update_role'])) {
    ...
    if ($stmt->execute()) {
        $_SESSION['status'] = "Rol başarıyla güncellendi.";
    } else {
        $_SESSION['status'] = "Rol güncellenemedi.";
    }
...
// Kullanıcıları listelemek için veritabanından verileri al
...
<body>
    ...
    <div class="content">
        ...
        <?php if (isset($_SESSION['status'])): ?>
            <div id="status-alert" class="alert <?php echo strpos($_SESSION['status'], 'başarıyla') !== false ? 'alert-success' : 'alert-danger'; ?>">
            ...
            <div class="row justify-content-center">
                <div class="col-md-4 form-container">
                    <form action="admin_users.php" method="POST">
                        ...
                        <div class="form-group">
                            <input type="text" class="form-control" name="s_number" placeholder="Okul Numarası" required />
                        </div>
                        <div class="form-group">
                                    ...
                                    <td>
                                        <form action="admin_users.php" method="POST">
                                            ...
                                            <select name="account_status" class="form-control" onchange="this.form.submit()">
                                                <option value="0" <?php echo $row['account_status'] == 0 ? 'selected' : ''; ?>>Yok</option>
                                                <option value="1" <?php echo $row['account_status'] == 1 ? 'selected' : ''; ?>>Var</option>
                                            </select>
                                            <input type="hidden" name="update_status" value="1">
                                        </form>
                                    </td>
                                    <td class="user-actions">
                                        <form action="delete_user.php" method="POST" onsubmit="return confirm('Bu kullanıcıyı silmek istediğinizden emin misiniz?');">
                                            <input type="hidden" name="user_id" value="<?php echo $row['id']; ?>">
